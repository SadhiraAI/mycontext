...
class CustomTemplate(Base):
    ...
    constraints: Mapped[str | None] = mapped_column(Text, nullable=True)  # JSON: must_include, must_not_include
    thinking_strategy: Mapped[str | None] = mapped_column(String(50), nullable=True, default="direct")
    examples: Mapped[str | None] = mapped_column(Text, nullable=True)  # JSON: [{input, output}]
    output_schema: Mapped[str | None] = mapped_column(Text, nullable=True)  # JSON: [{name, type}]
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
