...
def test_build_context():
    ...
    qa = QuestionAnalyzer()
    context = qa.build_context(question="What is the best approach?", depth="comprehensive")
...
def test_quality_metrics():
    ...
    context = Context(
        guidance=Guidance(role="Expert Analyst", rules=["Be clear", "Be thorough"]),
...
def test_context_creation():
    ...
    # Object creation
    context2 = Context(guidance=Guidance(role="Analyst"), directive=Directive(content="Analyze"))
...
def test_question_analyzer_execution():
    ...
    qa = QuestionAnalyzer()
    context = qa.build_context(question="How can I improve performance?", depth="standard")
...
def test_decision_framework_execution():
    ...
    context = df.build_context(
        decision="Choose cloud provider", options=["AWS", "Google Cloud", "Azure"]
    )
...
def test_idea_generator_execution():
    ...
    ig = IdeaGenerator()
    context = ig.build_context(challenge="Increase user engagement", constraints=["Budget: $10K"])
...
def test_context_assembly():
    ...
    context = Context(
        guidance=Guidance(
            role="Expert Data Scientist", rules=["Use statistical methods", "Visualize findings"]
        ),
        directive=Directive(
            content="Analyze customer churn data and identify key drivers", priority=5
        ),
    )
...
def test_pattern_parameters():
    ...
    context1 = ti.build_context(
        data_description="Monthly sales", domain="e-commerce", timeframe="past year"
    )
    assert context1 is not None

    ps = PrioritySetter()
    context2 = ps.build_context(items=["Task A", "Task B", "Task C"], goal="Optimize delivery")
    assert context2 is not None
...
def test_foundation_classes():
    ...
    guidance = Guidance(role="Test Role", rules=["Rule 1", "Rule 2"])
    assert guidance.role == "Test Role"
    assert len(guidance.rules) == 2

    constraints = Constraints(must_include=["Item 1"], must_not_include=["Item 2"])
